...
export class LumoSession {
   ...
   */
  async chat({ prompt = '', imageData = null, model = 'lumo-max', reasoning = false, onChunk = null, timeoutMs = 120000, visionRetries = 4, deadlineMs = null } = {}) {
    ...
    const sysMsg = imageData ? [{ role: 'system', content: VISION_SYS_PROMPT }] : [];
    const buildMessages = (attempt) => {
      ...
      if (attempt === 0) {
        return [...sysMsg, { role: 'user', content: textPart ? [textPart, ...imgPart] : imgPart }];
      }
      if (attempt === 1) {
        return [...sysMsg, { role: 'user', content: textPart ? [...imgPart, textPart] : imgPart }];
      }
      return [
        ...sysMsg,
